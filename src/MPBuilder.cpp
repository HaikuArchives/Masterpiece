	...
	:	BWindow(BRect(100, 100, 900, 700), windowTitle, B_DOCUMENT_WINDOW, B_ASYNCHRONOUS_CONTROLS | B_AUTO_UPDATE_SIZE_LIMITS, B_CURRENT_WORKSPACE), launcherMessage(msg), launcherMessenger(msgr)
{
	...
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	rightButton = new BButton(r, NULL, ">", new BMessage(MOVE_RIGHT), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
	r.left = rightButton->Frame().left;
	r.top = rightButton->Frame().bottom + 5;
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	leftButton = new BButton(r, NULL, "<", new BMessage(MOVE_LEFT), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
	r.left = leftButton->Frame().left;
	r.top = leftButton->Frame().bottom + 5;
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	topButton = new BButton(r, NULL, "TOP", new BMessage(MOVE_TOP), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
	r.left = topButton->Frame().left;
	r.top = topButton->Frame().bottom + 5;
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	upButton = new BButton(r, NULL, "UP", new BMessage(MOVE_UP), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
	r.left = upButton->Frame().left;
	r.top = upButton->Frame().bottom + 5;
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	downButton = new BButton(r, NULL, "DOWN", new BMessage(MOVE_DOWN), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
	r.left = downButton->Frame().left;
	r.top = downButton->Frame().bottom + 5;
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	bottomButton = new BButton(r, NULL, "BOTTOM", new BMessage(MOVE_BOTTOM), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
	r.left = bottomButton->Frame().left;
	r.top = bottomButton->Frame().bottom + 5;
	r.right = r.left + 80;
	r.bottom = r.top + 25;
	deleteButton = new BButton(r, NULL, "DELETE", new BMessage(DELETE_BUILDER_THT), B_FOLLOW_NONE, B_WILL_DRAW | B_NAVIGABLE);
	r = Bounds();
