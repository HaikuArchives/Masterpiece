...
MPPreview::MPPreview(const BMessage &msg, const BMessenger &msgr, BString windowTitle, int ideaID)
...
void MPPreview::MessageReceived(BMessage* msg)
{
	...
	switch(msg->what)
	{
		...
		case MENU_NEW_THT: // open another untitled editor window
			tmpEditor = new MPPreview(BMessage(SHOW_LAUNCHER), BMessenger(this), "MasterPiece Editor - untitled", -1);
			tmpEditor->Show();
			break;
