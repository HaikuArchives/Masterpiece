...
: BControl(frame,name,label,message,resizing_mode,B_WILL_DRAW|B_FRAME_EVENTS)
{
	...
	m_text_view = new MultiLineTextControlTextView(m_entry_text_rect.InsetByCopy(2,2),flags);
	if(text)
...
//******************************************************************************************************
MultiLineTextControlTextView::MultiLineTextControlTextView(BRect frame, uint32 flags)
: BTextView(frame,NULL,frame.OffsetToCopy(0,0).InsetByCopy(1,1),B_FOLLOW_ALL_SIDES,flags)
{
	m_tab_allowed = true;
	m_modified = false;
	SetWordWrap(true);
